...
assignCheck = [asInt, asChr, asBool, asDouble, asStr, asId]
 where
    ...
    asInt = (Program [TLAssign (Assign (NormalType "int") (Ident "x") (ExpLit (Number (Left 20))))], 
            parseSource "int x = 20;")
    -- |Check char literal assignment
    asChr = (Program [TLAssign (Assign (NormalType "char") (Ident "y") (ExpLit (Ch 'c')))], 
            parseSource "char y = 'c';")
    -- |Check bool literal assignment 
    asBool = (Program [TLAssign (Assign (NormalType "bool") (Ident "b") (ExpLit (Bl False)))], 
            parseSource "bool b = false;")
    -- |Check float literal assignment
    asDouble = (Program [TLAssign (Assign (NormalType "double") (Ident "d") (ExpLit (Number (Right 20.4))))], 
            parseSource "double d = 20.4;")
    -- |Check string literal assignment
    asStr = (Program [TLAssign (Assign (NormalType "string") (Ident "s") (ExpLit (Str "hi")))], 
            parseSource "string s = \"hi\";")
    -- |Check identifier assignment
    asId = (Program [TLAssign (Assign (NormalType "int") (Ident "i") (ExpIdent (Ident "x")))], 
            parseSource "int i =  x;")
...
binOpCheck = [asMul, asEq, asShift, asPrece, asPrece2, parensCheck]
 where
    ...
    asPrece = (Program [TLAssign (Assign (NormalType "int") (Ident "j")
              (ExpBinOp Add (ExpIdent (Ident "a")) (ExpBinOp Mul (
                ExpIdent (Ident "b")) (ExpIdent (Ident "c")))))]
              ,parseSource "int j = a + b * c;") 

    asPrece2 = (Program [TLAssign (Assign (NormalType "int") (Ident "k")
    ...
    parensCheck = (Program [TLAssign (Assign (NormalType "int")  (Ident "l")
...
unOpCheck = [asNot, asPrece, asPrece2, asParens]
 where
    ...
    asNot = (Program [TLAssign (Assign (NormalType "bool") (Ident "i") 
    ...
    -- Check precedence with binary operators   
    asPrece2 = (Program [TLAssign (Assign (NormalType "int") (Ident "j")
             (ExpBinOp Add (ExpUnaryOp BNot (ExpIdent (Ident "a"))) (ExpIdent (Ident "b"))))]
             ,parseSource "int j = ~ a + b;")
    -- Check precedence with parenthesis
    asParens = (Program [TLAssign (Assign (NormalType "int")  (Ident "k")
...
funCallCheck = [noArgs, singleArgs, multiArgs, multiComplexArgs]
 where
    ...
    multiComplexArgs = (Program [ fun $ [AssignStmt $ (Assign (NormalType "int") (Ident "a")
    ...
    fun xs = Func (NormalType "tes") (Ident "test") [] (BlockStmt xs)
    funStr = "tes test() {"
...
seqParBlockCheck = [seqB, parB, seqMultiB]
 where
    seqB = (Program [fun [(Seq $ BlockStmt [AssignStmt $ Assign 
            (NormalType "int") (Ident "i") (ExpIdent (Ident "x"))])]],
    ...
    seqMultiB = (Program [fun [ (Seq $ BlockStmt [AssignStmt $ Assign 
            (NormalType "int") (Ident "i") (ExpIdent (Ident "x")),
           AssignStmt $ Assign (NormalType "int") (Ident "j") (ExpIdent (Ident "y"))])]],
